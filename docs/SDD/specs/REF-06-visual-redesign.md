...
| **D2 — Desktop layout** | **L1 — full-bleed**: board centered and dominant; HUD strip floats above the board. **Revised in Phase A.2:** secondary information leaves the board surface entirely. Active power-ups become a vertical buff rail fixed in the top-left of the **viewport** (not the board); combo display becomes a glowing badge fixed in the top-right of the **viewport**, gated to appear only when `combo.count >= COMBO_CONFIG.minCombo`; phase / level progress sits in a slim horizontal info bar **below** the board card, sharing its width via `align-items: stretch` inside a `.boardStack` flex column. No left/right rails, **and no overlay invades the playfield**. | HUD canon from Number Analytics ("critical info prominently displayed"). Removes dashboard look. Phase A.2 revision: per owner direction, the playfield surface itself stays 100 % clean — visual references *Devil May Cry / Bayonetta* (combo badge), *WoW / FF14* (buff rail), *Witcher 3 / Hades* (objective tracker). Aligns with US-02 and SR-01. |
