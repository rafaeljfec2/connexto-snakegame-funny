...
const getParticleCount = (id: number, mobile: boolean): number => {
  ...
  switch (id) {
    case 1: return Math.floor(60 * multiplier); // Stars
    case 2: return Math.floor(20 * multiplier); // Fog
    case 3: return Math.floor(80 * multiplier); // Sand
    case 4: return Math.floor(50 * multiplier); // Cosmic
    case 5: return Math.floor(60 * multiplier); // Fire/Embers
    case 6: return Math.floor(40 * multiplier); // Chaos
    case 7: return Math.floor(30 * multiplier); // Geometric/Mist
    case 8: return Math.floor(50 * multiplier); // Ash/Lava
    case 9: return Math.floor(100 * multiplier); // Rain (Storm)
    case 10: return Math.floor(50 * multiplier); // Divine
    default: return 0;
  }
};
...
const createParticle = (id: number, reset: boolean = false): Particle => {
  ...
  const y = reset ? (id === 9 || id === 3 || id === 5 ? -10 : Math.random() * height) : Math.random() * height;
...
const updateParticles = () => {
  particles.forEach(p => {
    ...
    if (phaseId === 5 || phaseId === 10) { // Upward movement
        if (p.y < -50) {
            Object.assign(p, createParticle(phaseId, true));
        }
    } else { // Downward/General movement
